...
if block_size < model_block2.config.block_size:
    ...
    model_args['block_size'] = block_size # so that the checkpoint will have the right value
if block_size < model_block3.config.block_size:
    model_block3.crop_block_size(block_size)
    model_args['block_size'] = block_size # so that the checkpoint will have the right value
if block_size < model_block4.config.block_size:
...
# optimizer
optimizer_1 = model_block1.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)
optimizer_2 = model_block2.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)
optimizer_3 = model_block3.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)
optimizer_4 = model_block4.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)
optimizer_5 = model_block5.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)
optimizer_6 = model_block6.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)
optimizer_7 = model_block7.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)
optimizer_8 = model_block8.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)
...
def estimate_loss():
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            X, Y = get_batch(split)
            with ctx:
                logits, loss, x_feature = model_block1(X, Y)
                logits, loss, x_feature = model_block2(x_feature, Y)
                logits, loss, x_feature = model_block3(x_feature, Y)
                logits, loss, x_feature = model_block4(x_feature, Y)
                logits, loss, x_feature = model_block5(x_feature, Y)
                logits, loss, x_feature = model_block6(x_feature, Y)
                logits, loss, x_feature = model_block7(x_feature, Y)
                logits, loss, x_feature = model_block8(x_feature, Y)
            losses[k] = loss.item()
...
raw_model_block5 = model_block5 # unwrap DDP container if needed
raw_model_block6 = model_block6 # unwrap DDP container if needed
raw_model_block7 = model_block7 # unwrap DDP container if needed
raw_model_block8 = model_block8 # unwrap DDP container if needed

running_mfu = -1.0

log_file = "8-block-ours-training-log.txt"
with open(log_file, "w", encoding="utf-8") as file:
    file.write("Iteration\tTraining Loss\tValidation Loss\n")
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        with open(log_file, "a", encoding="utf-8") as file:
            file.write(f"{iter_num}\t{losses['train']:.4f}\t{losses['val']:.4f}\n")
    ...
    for micro_step in range(gradient_accumulation_steps):
        with ctx:
            ...
            loss_5 = loss_5 / gradient_accumulation_steps # scale the loss to account for gradient accumulation
            loss_6 = loss_6 / gradient_accumulation_steps # scale the loss to account for gradient accumulation
            loss_7 = loss_7 / gradient_accumulation_steps # scale the loss to account for gradient accumulation
            loss_8 = loss_8 / gradient_accumulation_steps # scale the loss to account for gradient accumulation
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        lossf = loss_8.item() * gradient_accumulation_steps
